...
########################
plot.HubNetwork = function(corResult,probResult){
  NetworkDf = cbind(
    corResult[,c("Hub", "Interactor")], 
    P.value = probResult[match(corResult[,"Hub"], probResult[,"Hubs"]),"P.value"])
  
  rownames(NetworkDf) = NULL
  
  
  
  graph = graph.data.frame(NetworkDf[NetworkDf[,"Interactor"] %in% NetworkDf[,"Hub"],], directed = F)
  graph = simplify(graph)
  plotVnames = V(graph)$name
  
  vPvalue = NetworkDf[match(plotVnames,NetworkDf[,"Hub"]),"P.value"]
  vPvalue[vPvalue==min(vPvalue)] = 0.001
  
  # Pvalue of the vertices, only hubs will get a Pvalue. 
  set.seed(1)
  result = plot.igraph(graph,
                       vertex.size = -2*log(vPvalue),
                       vertex.color = ifelse(vPvalue<=0.05,"red","#ADD8E6"),
                       vertex.label = NA,
                       layout = layout.lgl)
  return(result)
} # End plot.HubNetwork
