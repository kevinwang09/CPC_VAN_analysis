...
identifySignificantHubs = function(exprFile, labelIndex, mapFile, outFile
                                   ...
                                   , exprDataType="SYMB", ppiDataType="SYMB", outputDataType="SYMB"
                                   , species="Human", inputCores=4
                                   , hubVect = NULL, interactomeVect = NULL, RunShinyAPP = FALSE) {
  ################## End Changes ###################
  ...
  ##################################################
  if (RunShinyAPP == TRUE) {require(shiny)}
  ################## End Changes ###################
	...
	##################################################
	message(paste("Number of threads = ", getDoParWorkers(), sep=""))
	################## End Changes ###################
...
# 					, outFile, changeToGeneSymb, twoInputExprFiles, uniqueLabels)	
	##################################################
	interactomeAnalysisResult = interactomeAnalysis(filterSrcExprMatrix, filterSrcRegMatrix, exprHubs, randomizeCount, assocType, numCores
	                                                , outFile, changeToGeneSymb, twoInputExprFiles, uniqueLabels, RunShinyAPP)
	
	corResult = interactomeAnalysisResult[["corVect"]]
	probVect = interactomeAnalysisResult[["probVect"]]
	################## End Changes ###################
	...
	##################################################
	probResult = saveProbValues(probVect, elementsInExprHub, totalElementsInExprHub, outFile, changeToGeneSymb, twoInputExprFiles, adjustMethod)
	ResultList = list(corResult,probResult)
	names(ResultList) = c("corResult", "probResult")
	return(ResultList)
	##################################################
	stopCluster(cl)
}
...
interactomeAnalysis = function(inExprMatrix, inRegMatrix, inHubList, inCount, assocType, coreCount, fileName
				, getGeneSymb, isMicro, sampleGroups, RunShinyAPP) {
	...
	if(numUniqueLabels == 2) {
		...
		################################################
		corVect = matrix(numeric(0),ncol=5)
		# colnames(corVect) = c("Hub", "Interactor", sampleGroups[1], sampleGroups[2], corLabel)
		# The colnames does not work, for structure reasons...
		################################################
	}
	else {
	  ############## Orignal Code #######################
	  # write(c("Hub", "Interactor", sampleGroups), corFile, sep="\t", ncolumns=numUniqueLabels+2)
	  ###################################################
	  corVect = matrix(rep(numeric(0)),ncol=numUniqueLabels+2)
	  colnames(corVect) = c("Hub", "Interactor", sampleGroups)
	  ###################################################
	}
	...
	###################################################
	message(paste("Number of hubs = ", length(inHubList), sep=""))
	###################################################
	...
	for(hubIndex in 1:length(inHubList)) {
	  ...
	  ###################################################
	  message(paste("Current hub = ", hubIndex, sep=""))
	  if (RunShinyAPP == TRUE) {incProgress(1/length(inHubList))}
	  ###################################################
...
# 				, corFile, getGeneSymb, isMicro)
		########################################
		corVect = rbind(corVect,saveCorValues(currentHub, currentInteractors, origHubDiff$corMatrix
		              , corFile, getGeneSymb, isMicro, sampleGroups))
  	########################################
		...
		# 				, "interactomeSpearmanCorrelation", "getSpearmanCor"
		...
		exportFunctions = c("calculateProbDist", "getCorrelation"
		                    ...
		                    , "interactomePearsonCorrelation", "getPearsonCor"
		                    , "interactomeSpearmanCorrelation", "getSpearmanCor"
		                    , "interactomeDistanceCorrelation", "getDistanceCor"
		                    , "interactomeMICCorrelation", "getMICCor"
		                    , "getSampleIndexes", "getFStat")
		########################################
...
####################################################		
		probTemp = foreach(i = 1:coreCount, .combine='c', .export = exportFunctions, .packages = c("energy", "minerva")) %dopar% {
####################################################				  
	...
	###########################################
	Result = list(corVect,probVect)
	names(Result) = c("corVect","probVect")
	return(Result)
	###########################################
}
...
getCorrelation = function(inVect, inMatrix, sampleGroups, corType, corInfo, permuteLabels) {
	...
	# 	
	# 	}		
	# Since length(sampleGroups) was used, there is potential for this function to be applied on more than 2 uniqueLabels.
	# But so far, I don't want to risk it, if the corType is FSTAT, it should go to PCC. 
	######################################################
	if(corType == "Spearman") { ## Spearman CC
	  for(i in 1:length(sampleGroups)) corVectMatrix[, i] = interactomeSpearmanCorrelation(inVect[labelList[[i]]], inMatrix[, labelList[[i]]])
	}
	
	if(corType == "Distance") { ## Distance Correlation Bias-corrected
	  for(i in 1:length(sampleGroups)) corVectMatrix[, i] = interactomeDistanceCorrelation(inVect[labelList[[i]]], inMatrix[, labelList[[i]]])
	}
	
	if(corType == "MIC") { ## MIC
	  for(i in 1:length(sampleGroups)) corVectMatrix[, i] = interactomeMICCorrelation(inVect[labelList[[i]]], inMatrix[, labelList[[i]]])
	}
	
	if(corType == "TCC" || corType == "PCC" || corType == "Spearman" || corType == "Distance" || corType == "MIC") {
...
################### Original Code ####################################
# Adding Spearman Coefficient
#######################################################################

interactomeSpearmanCorrelation = function(hubVector, geneMatrix) {	
  return(apply(geneMatrix, 1, getSpearmanCor, hubVector))	
}


getSpearmanCor = function(x, inVect) {
  return(cor(x, inVect, method = "spearman"))
}
...
################## Original Code ####################################
# Adding Distance Correlation
#######################################################################

interactomeDistanceCorrelation = function(hubVector, geneMatrix) {	
  return(apply(geneMatrix, 1, getDistanceCor, hubVector))	
}


getDistanceCor = function(x, inVect) {
  return(dcor.ttest(x, inVect)$estimate)
}
...
interactomeMICCorrelation = function(hubVector, geneMatrix) {	
  return(apply(geneMatrix, 1, getMICCor, hubVector))	
}


getMICCor = function(x, inVect) {
  return(mine(x, inVect)$MIC)
}
